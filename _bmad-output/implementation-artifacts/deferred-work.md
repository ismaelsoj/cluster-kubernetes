...
- **[AC2 — passthrough do `Authorization: Bearer`]** ✅ RESOLVIDO (2026-05-29): `userinfo` retorna 401 sem token e 200 com token → o Bearer original chega intacto ao upstream. Os flags `PASS/SET_AUTHORIZATION_HEADER=false` controlam injeção de ID-token, não o Bearer original — não havia contradição. (Revalidar quando a API real do Épico 4 entrar, já que aí o upstream muda do `userinfo` do Keycloak para a API de negócio.)
- **[AC2 — latência adicional < 20ms]** ✅ RESOLVIDO (2026-05-29): medido — overhead da validação ~2–5ms (cadeia completa mediana 6.51ms / p95 8.27ms vs baseline sem OAuth2-Proxy 4.16ms), bem abaixo dos 20ms.
